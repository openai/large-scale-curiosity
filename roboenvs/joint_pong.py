...
class PongScene(Scene):
    ...
    def episode_restart(self):
        ...
        if self.score_right + self.score_left > 0:
            ...
            sys.stdout.flush()
        self.mjcf = self.cpp_world.load_mjcf(os.path.join(os.path.dirname(__file__), "roboschool_pong.xml"))
        dump = 0
        for r in self.mjcf:
...
class RoboschoolPongJoint(gym.Env):
    VIDEO_W = 84
    VIDEO_H = 84
